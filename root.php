...
class penjualan
{
	...
	{
		$this->con = new mysqli("localhost", "root", "", "resnlight");
	}
	function __destruct()
	{
		$this->con->close();
...
?><script type="text/javascript">
	...
	function tambah_barang($id_barang, $nama_barang, $stok, $harga_beli, $harga_jual, $id_kategori)
	{
		...
		if ($query->num_rows > 0) {
			...
			$query2 = $this->con->query("insert into barang set id_barang='$id_barang',nama_barang='$nama_barang',id_kategori='$id_kategori',stok='$stok',harga_beli='$harga_beli',harga_jual='$harga_jual'");
	...
	function tampil_barang_stok_filter($tanggal, $aksi)
	{
		if ($keyword == "null") {
	...
	{
		$query = $this->con->query("select t.id_transaksi,t.tgl_transaksi,t.no_invoice,SUM(b.harga_beli* st.jumlah_beli) as modal,t.total_bayar,t.nama_pembeli,user.username 
			from transaksi t
		...
		?>
			<tr>
				...
				<td>Rp. <?= number_format($f['modal']) ?></td>
				<td>Rp. <?= number_format($f['total_bayar']) ?></td>
				<td>Rp. <?= number_format($f['total_bayar'] - $f['modal']) ?></td>
				<td>
...
<?php
	...
	function tambah_tempo($id_barang, $jumlah, $trx)
	{
		...
		if (!$data) {
			echo "<script>alert('Barang tidak ditemukan');window.location='transaksi.php?action=transaksi_baru';</script>";
			exit;
		}

		if ($data['stok'] < $jumlah) {
			echo "<script>alert('Stok tidak mencukupi');window.location='transaksi.php?action=transaksi_baru';</script>";
			exit;
		}

		// Cek apakah barang sudah ada di tempo
		$q = $this->con->query("SELECT * FROM tempo WHERE id_barang='$id_barang' AND trx='$trx'");
		if ($q->num_rows > 0) {
			$ubah = $q->fetch_assoc();
			$jumbel = $ubah['jumlah_beli'] + $jumlah;
			$total_harga = $jumbel * $data['harga_jual'];
			$dbquery = $this->con->query("UPDATE tempo SET jumlah_beli='$jumbel', total_harga='$total_harga' WHERE id_barang='$id_barang' AND trx='$trx'");
		} else {
			$total_harga = $jumlah * $data['harga_jual'];
			$dbquery = $this->con->query("INSERT INTO tempo (id_barang, jumlah_beli, total_harga, trx) VALUES ('$id_barang', '$jumlah', '$total_harga', '$trx')");
		}

		if ($dbquery === TRUE) {
			$this->con->query("UPDATE barang SET stok = stok - $jumlah WHERE id_barang = '$id_barang'");
			header("Location: transaksi.php?action=transaksi_baru");
			exit;
		} else {
			echo "<script>alert('Gagal menyimpan ke tempo');window.location='transaksi.php?action=transaksi_baru';</script>";
			exit;
		}
	}
